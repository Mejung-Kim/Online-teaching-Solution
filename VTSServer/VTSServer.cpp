...
// VTSServer.cpp : 응용 프로그램에 대한 클래스 동작을 정의합니다.
//
...
BOOL CVTSServerApp::InitInstance()
{
	...
	filepath = new char[200]; //파일 경로 초기화
	isTeacher = 0; //LoginDlg에서 선생님인지 학생인지에 대한 정보를 받아온다.ㅏ
	...
	if(nResponse == IDOK){
		...
		if(isTeacher){
			AfxMessageBox(_T("선생님 권한으로 로그인합니다."));
			CVTSServerDlg dlg;
			m_pMainWnd = &dlg;
			nResponse = dlg.DoModal();	
		}
		else{
			AfxMessageBox(_T("학생 권한으로 로그인합니다."));
			CVTSClntDlg clntdlg;
			m_pMainWnd = &clntdlg;
